...
<head>
    ...
    <title>Orders add to cart to day</title>
    <style>
        table {
...
// Retrieve customer and product data for today's orders
$sql = "SELECT customers.*, orderproducts.name AS product_name, orderproducts.price AS product_price, orderproducts.img AS product_img, orderproducts.in_cart AS product_in_cart, (orderproducts.price * orderproducts.in_cart) AS total_price, orderproducts.order_date AS order_date FROM customers INNER JOIN orderproducts ON customers.id = orderproducts.customer_id WHERE DATE(orderproducts.order_date) = CURDATE()";
